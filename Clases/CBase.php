...
class CRtf extends CBase {
   ...
   public $lcFolXls, $lcFolSal, $lcFilRet, $lcFilInp, $lcTodo, $lp, $laTexto;
   ...
   public function omInicializar() {
      $lcFile1 = 'Xls/'.$this->pcFile.'.rtf';
      if (!is_file($lcFile1)) {
         $this->pcError = 'ARCHIVO DE ORIGEN NO EXISTE';
         return false;
      }
      if (empty($this->pcFilRet)) {
         $this->pcFilRet = 'FILES/R'.rand().'.doc';
...
function fxInitSession() {
   ///if (!(isset($_SESSION["GCNOMBRE"]) and isset($_SESSION["GCUNIACA"]))) {  ////// FALTA ARREGLAR EL PŔOCEDIMIENTO ALMACENADO
if (!(isset($_SESSION["GCNOMBRE"]) )) {      
    return false;
   }
   return true;
}
